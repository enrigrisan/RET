...
for ct1=1:blobdark
   if dbf,
      disp([' Blob dark ',num2str(ct1),' of ',num2str(blobdark)]);
   end
   [r,c]=find(xbpdark==ct1);
   ind=sxbpwhite(1)*(c-1)+r;
   switch cldark(ct1)
      % For dark lesions the probability of being one of
      % the white lesions is always zero
   case 1
      xblob(ind)=ct2;
      nvf(ct2,1).ind=ct1;
      nvf(ct2,1).type='haemorrhage';
      nvf(ct2,1).p=[pdark(ct1,1);0;0;0;sum(pdark(ct1,2:3))];
      nvf(ct2,1).area=length(ind);
      nvf(ct2,1).center=round([mean(r);mean(c)]);
      ct2=ct2+1;
   end
