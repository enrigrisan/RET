function seg=RETsparsetrack(xroi,xs,ys,ds,nb,trackpar,lsegmin,rcont,doverlap,noverlap,dseed,dseedr,dbf),
            ...
            if length(xcv)>lsegmin,
                ...
                for ct2=1:length(segn),
                    ...
                    if length(xcv)>lsegmin,
                        ...
                        xscont1=fix(xcv(1)+rcont*cos(dirv(1)+pi));
                        yscont1=fix(ycv(1)+rcont*sin(dirv(1)+pi));
                        dscont1=dv(1);
                        sxcv=length(xcv);
                        xscont2=fix(xcv(sxcv)+rcont*cos(dirv(sxcv)));
                        yscont2=fix(ycv(sxcv)+rcont*sin(dirv(sxcv)));
                        dscont2=dv(sxcv);
                        xs=[xs;xscont1;xscont2];
                        ys=[ys;yscont1;yscont2];
                        ds=[ds;dscont1;dscont2];
